...
namespace NinjaDomain.DataModel
{
    public class DisconnectedRepository
    {
        ...
        public List<Ninja> GetNinjasWithClan()
        {
            using (var context = new NinjaContext())
            {
                //return context.Ninjas.Include(c => c.Clan).ToList();
                return context.Ninjas.AsNoTracking().Include(c => c.Clan).ToList();
            }
        }

        public List<NinjaEquipment> GetEquipments()
        {
            using (var context = new NinjaContext())
            {
                return context.Equipment.AsNoTracking().ToList();
            }
        }
        ...
        public void SaveUpdatedEquipment(NinjaEquipment equipment)
        {
            //paying the price of not having a foreign key here.
